...
async function main() {
  ...
  if (isTech) {
    ...
    document.querySelectorAll(".side-nav")?.forEach((el) => el.classList.add("d-none"));
  }
  ...
  const serie = serieParam;
  const idClienteParam = getParam("idCliente") || "";
  const incidenteId = getParam("id");

  const scopeClienteId = scope?.clienteId ? String(scope.clienteId) : "";
  const scopeUsuarioId = scope?.idUsuario ? String(scope.idUsuario) : "";
  if (scopeClienteId) {
    form.idCliente.value = scopeClienteId;
    form.idCliente.readOnly = true;
  }
  if (scopeUsuarioId) {
    form.idUsuario.value = scopeUsuarioId;
    document.getElementById("res-usuario").textContent = `Usuario ${scopeUsuarioId}`;
    form.idUsuario.readOnly = true;
  }

  form.serieEquipo.value = serie;
  if (form.serieEquipo.value) form.serieEquipo.readOnly = true;
  ...
  form.addEventListener("submit", async (ev) => {
    ...
    try {
      ...
      setTimeout(() => (window.location.href = targetReturn), 500);
    } catch (err) {
