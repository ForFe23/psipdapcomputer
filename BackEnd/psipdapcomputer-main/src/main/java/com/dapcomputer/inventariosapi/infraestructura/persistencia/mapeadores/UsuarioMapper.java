...
public interface UsuarioMapper {
    default Usuario toDomain(UsuarioJpa origen) {
        ...
        if (origen.getRol() != null) {
            rol = new Rol(origen.getRol().getId(), origen.getRol().getCodigo(), origen.getRol().getNombre(), origen.getRol().getEstadoInterno());
        } else if (origen.getRolId() != null) {
            rol = new Rol(origen.getRolId(), null, null, null);
        }
        return new Usuario(
                origen.getIdCliente(),
                origen.getEmpresaId(),
