...
public class RegistrarActaServicio implements RegistrarActaCasoUso {
    ...
    public Acta ejecutar(Acta acta) {
        ...
        List<ActaItem> items = prepararItemsBase(acta);
    ...
    private List<ActaItem> prepararItemsBase(Acta acta) {
        if (acta.items() != null && !acta.items().isEmpty()) {
            return acta.items();
        }
        if (acta.idEquipo() == null && (acta.equipoSerie() == null || acta.equipoSerie().isBlank())) {
            return acta.items();
        }
        List<ActaItem> items = new ArrayList<>();
        ActaItem base = new ActaItem(
                null,
                acta.id(),
                1,
                acta.equipoTipo() != null ? acta.equipoTipo() : "EQUIPO",
                acta.equipoSerie(),
                acta.equipoModelo(),
                "",
                "Equipo entregado",
