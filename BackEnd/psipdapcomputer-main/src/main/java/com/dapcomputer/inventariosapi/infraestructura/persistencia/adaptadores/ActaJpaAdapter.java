...
public class ActaJpaAdapter implements ActaRepositorio {
    ...
    @Override
    @Transactional(readOnly = true)
    public Optional<Acta> buscarPorEquipo(Integer idEquipo) {
        if (idEquipo == null) return Optional.empty();
        return repository.findFirstByIdEquipoOrderByIdDesc(idEquipo).map(mapper::toDomain);
    }
}
