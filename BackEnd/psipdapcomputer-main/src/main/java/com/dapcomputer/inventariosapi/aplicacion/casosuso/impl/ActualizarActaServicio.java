...
public class ActualizarActaServicio implements ActualizarActaCasoUso {
    ...
    public ActualizarActaServicio(ActaRepositorio repositorio, ActaItemRepositorio actaItemRepositorio, SincronizarMovimientoPorActaCasoUso sincronizarMovimientoPorActaCasoUso) {
    ...
    public Acta ejecutar(Acta acta) {
        ...
        if (existente.estado() == EstadoActa.CERRADA || existente.estado() == EstadoActa.ANULADA) {
            throw new IllegalStateException("El acta está cerrada o anulada y no admite edición");
        }
        ...
        if (estado == EstadoActa.ANULADA) {
            actaItemRepositorio.actualizarEstadoInternoPorActa(guardado.id(), "INACTIVO_INTERNAL");
        }
