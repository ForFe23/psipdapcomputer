...
public interface ActaDtoMapper {
    ...
    default List<ActaItem> toDomainItems(ActaDto origen) {
        ...
        return origen.items().stream()
                .map(item -> new ActaItem(item.id(), actaId, item.itemNumero(), item.tipo(), item.serie(), item.modelo(), item.marca(), item.observacion(), item.equipoId(), item.equipoSerie(), item.estadoInterno()))
                .toList();
    }
    ...
    default List<ActaItemDto> toDtoItems(Acta origen) {
        ...
        return origen.items().stream()
                .map(item -> new ActaItemDto(item.id(), item.itemNumero(), item.tipo(), item.serie(), item.modelo(), item.marca(), item.observacion(), item.equipoId(), item.equipoSerie(), item.estadoInterno()))
                .toList();
    }
}
